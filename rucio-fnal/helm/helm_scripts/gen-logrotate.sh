...
if [ -z "$FNAL_RUCIO_DIR" ]; then
    ...
    helm template --name rucio-$EXPERIMENT-logrotate $FNAL_RUCIO_DIR/rucio-fnal/helm/helm-fnal/logrotate \
        --set experiment=$EXPERIMENT \
        --set image.tag=$FNAL_RUCIO_VERSION_TAG \
        -f $FNAL_RUCIO_DIR/$EXPERIMENT/helm/logrotate/values.yaml > $FNAL_RUCIO_DIR/$EXPERIMENT/logrotate.yaml
fi
