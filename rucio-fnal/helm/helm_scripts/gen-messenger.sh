...
if [ -z "$FNAL_RUCIO_DIR" ]; then
    ...
    helm template --name rucio-$EXPERIMENT-messenger $FNAL_RUCIO_DIR/rucio-fnal/helm/messenger -f $FNAL_RUCIO_DIR/$EXPERIMENT/helm/messenger/values.yaml > $FNAL_RUCIO_DIR/$EXPERIMENT/messenger.yaml
fi
