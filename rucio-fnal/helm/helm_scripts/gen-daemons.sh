...
if [ -z "$FNAL_RUCIO_DIR" ]; then
    ...
    helm template --name rucio-$EXPERIMENT $FNAL_RUCIO_DIR/rucio-fnal/helm/helm-charts/charts/rucio-daemons \
        --set experiment=$EXPERIMENT \
        --set image.tag=$FNAL_RUCIO_VERSION_TAG \
        -f $FNAL_RUCIO_DIR/$EXPERIMENT/helm/daemons/values.yaml \
        -f $FNAL_RUCIO_DIR/$EXPERIMENT/helm/secret/config.yaml > $FNAL_RUCIO_DIR/$EXPERIMENT/daemons.yaml
fi
