...
if [ -z "$FNAL_RUCIO_DIR" ]; then
    ...
    helm template --name rucio-$EXPERIMENT-filebeat $FNAL_RUCIO_DIR/rucio-fnal/helm/helm-fnal/filebeat --set experiment=$EXPERIMENT -f $FNAL_RUCIO_DIR/$EXPERIMENT/helm/filebeat/values.yaml > $FNAL_RUCIO_DIR/$EXPERIMENT/filebeat.yaml
fi
