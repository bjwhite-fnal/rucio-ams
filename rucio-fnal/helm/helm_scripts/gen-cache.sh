...
if [ -z "$FNAL_RUCIO_DIR" ]; then
    ...
    helm template --name rucio-$EXPERIMENT-cache $FNAL_RUCIO_DIR/rucio-fnal/helm/cache -f $FNAL_RUCIO_DIR/$EXPERIMENT/helm/cache/values.yaml > $FNAL_RUCIO_DIR/$EXPERIMENT/cache.yaml
fi
