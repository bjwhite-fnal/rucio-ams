...
if [ -z "$FNAL_RUCIO_DIR" ]; then
    ...
    helm template --name rucio-$EXPERIMENT-statsd $FNAL_RUCIO_DIR/rucio-fnal/helm/helm-fnal/statsd \
        --set experiment=$EXPERIMENT \
        --set image.tag=$FNAL_RUCIO_VERSION_TAG \
        -f $FNAL_RUCIO_DIR/$EXPERIMENT/helm/statsd/values.yaml > $FNAL_RUCIO_DIR/$EXPERIMENT/statsd.yaml
fi
