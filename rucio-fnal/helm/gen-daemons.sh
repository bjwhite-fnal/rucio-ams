...
if [ -z "$FNAL_EXP_RUCIO_CONF_DIR" ]; then
    ...
    helm template --name rucio-$EXPERIMENT $RUCIO_HELM_TEMPLATE_DIR/daemons -f $FNAL_EXP_RUCIO_CONF_DIR/helm/daemons/values.yaml -f $FNAL_EXP_RUCIO_CONF_DIR/helm/secret/config.yaml > $FNAL_EXP_RUCIO_CONF_DIR/daemons.yaml
fi
