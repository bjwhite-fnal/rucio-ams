...
if [ -z "$RUCIO_AMS_DIR" ]; then
    ...
    helm template --release-name rucio-$EXPERIMENT $RUCIO_AMS_DIR/rucio-ams/helm/helm-charts/charts/rucio-server \
        ...
        -f $RUCIO_AMS_DIR/$EXPERIMENT/helm/secret/config.yaml > $RUCIO_AMS_DIR/$EXPERIMENT/server.yaml
fi
