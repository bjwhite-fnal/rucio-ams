...
if [ -z "$RUCIO_AMS_DIR" ]; then
    ...
    helm template --release-name rucio-$EXPERIMENT-prometheus $RUCIO_AMS_DIR/rucio-ams/helm/helm-fnal/prometheus \
        --set experiment=$EXPERIMENT \
        --set image.tag=$RUCIO_AMS_VERSION_TAG \
        -f $RUCIO_AMS_DIR/$EXPERIMENT/helm/prometheus/values.yaml > $RUCIO_AMS_DIR/$EXPERIMENT/prometheus.yaml
fi
